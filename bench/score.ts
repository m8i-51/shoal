...
export interface BenchScoreEntry {
  ...
  runDate: string;
  config?: string;
  /** matched findings / total findings, 0..1. Absent on entries scored before this field existed. */
  precision?: number;
...
export interface BenchResult {
  ...
  precision: number; // 0..1 — (totalFindings - unmatchedFindings) / totalFindings
}
...
export function scoreFindings(findings: Finding[], labels: BenchLabel[]): BenchResult {
  ...
  return {
    ...
    unmatchedFindings,
    precision: findings.length > 0 ? (findings.length - unmatchedFindings) / findings.length : 0,
  };
}
...
export function formatBenchResult(result: BenchResult, variantId = "store"): string {
  const lines = [
    ...
    `Precision: ${Math.round(result.precision * 100)}% (${result.totalFindings - result.unmatchedFindings}/${result.totalFindings} findings matched a seeded label)`,
...
export function formatPublishedScoresMarkdown(scores: PublishedBenchScores): string {
  ...
  for (const entry of scores.entries) {
    const precision = entry.precision !== undefined ? `${Math.round(entry.precision * 100)}%` : "—";
