...
テストスイートが見るのは「誰かが確認しようと思いついたこと」だけ。ユーザーに
届いてしまうバグは、たいてい誰もテストを書かなかった経路にある — 動くけれど
わかりにくいフロー、誰もデザインしなかった空状態、項目が空だと落ちる管理画面。

shoal は、稼働中の Web アプリに AI エージェントの群れを放つ。各エージェントは
固有のペルソナを持ち、実際のユーザーのようにページを開き、操作し、迷い、気づく。
最後にトリアージが重複をまとめて issue チケットとして起票する。テストスクリプト
不要、テストデータ不要、アプリの事前知識も不要。URL を渡すだけ。
...
ステージングやプレビュー環境があり、手で書く時間のない探索的テストのカバレッジ
が欲しいチーム。アプリ全体を週次で一巡し、見つけたものを直せる人の目の前に
起票するところまでを自動でやる。
...
- **1 run ごとに実費がかかる。** エージェントの 1 ターンが 1 回の LLM 呼び出し。
  `SHOAL_MAX_USD` で run 単位の上限を設定でき、価格が公開されていないモデルで
  上限が発火できない場合は `shoal doctor` が警告する。
- **見つかるのは一部で、全部ではない。** shoal 自身のベンチマーク（バグを仕込んだ
  サンプルアプリと正解ラベル）での実測検出率は **71%**（仕込んだバグ 7 件、
  `claude-sonnet-4`）。[shoal-bench](#shoal-bench) 参照 — 自分でも回せる。
- **2 回走らせると結果が違う。** エージェントは LLM 駆動なので、これは探索であって
  リグレッションスイートではない。カバレッジは run をまたいで記録され、次の run
  は未探索の領域に向かう。
- **誤検出は出る。** トリアージが重複をまとめ、既存 issue と重複するものはスキップ
  するが、最後に判断するのは人間。ベンチが検出率と併せて precision を出すのは
  まさにこのため。
- **コードは一切読まない。** ユーザーから見えるものしか見ていない。それが狙いで
  あり、同時に限界でもある — 「このフローが壊れている」は言えるが「どのコミットが
  壊したか」は言えない。

決定的なテストスイートを**補う**ものであって、置き換えるものではない。
...
**行動の契約** — ペルソナは経歴の文章ではない。ページの見方、チュートリアルを飛ばすか全部読むか、知らないこと、混乱が増える条件、タブを閉じる条件まで書く。同じ 4 ステップのオンボーディングで二人の行動が分岐すること。ダッシュボードの種・`personas.yaml`・ペルソナデザイナーがこの契約を書き、ブラウザ／API エージェントは「丁寧なテスターであれ」と矛盾しても契約を優先する。

**初見（画面だけ）** — 契約は、その人が*知っていること*も引ける。初見のブラウザエージェントには仕様も機能一覧もコンソールもアクセシビリティツリーも渡さない。画面が製品だ。探索者は今までどおりブリーフィングを持つ。Personas パネルで人ごとに、開始ダイアログでランのブラウザ全体を切り替えられる（混在 / 全員初見 / 全員探索者）。API としきい値は常に探索者。プロダクト理解・ゴール・尖りは切れない。
...
GitHub Issues・Jira・Notion・Backlog・Asana など複数のトラッカーへの同時起票、または手元の HTML レポートとして確認できる。**Web ダッシュボード**でランの開始・進行状況のリアルタイム監視・Finding のカテゴリ別確認・トリアージ結果の確認・採用率と LLM コストの横断集計が行える。

---

## プロダクトの尖り — 直すと平均に寄ってしまう指摘

多様なペルソナがそれぞれ「自分のようなユーザーが期待するもの」を指摘する仕組みは、
放っておくとプロダクトを平均に近づける装置でもある。指摘を全部そのまま直していくと、
尖らせて作ったところがカテゴリの「ふつうの形」に収束していく。

そこで shoal では、このプロダクトが意図的に尖らせているところと、そのために捨てて
いるものを宣言できる:

```
尖り            全操作をキーボードで完結させる。マウス動線は意図的に用意しない
トレードオフ    オンボーディングは用意しない。訓練された担当者が使う前提
```

草案はプロダクトディスカバリがアプリ自体から推論する。ダッシュボードの
**プロダクトの尖り**パネルで直せば、それ以降の再ディスカバリでも上書きされない。

triage は指摘をこれまでどおり起票する（握りつぶしも骨抜きもしない）。ただし、
素直に直すと宣言した尖りが鈍る場合は **`edge-risk`** ラベルを付け、何が懸かって
いて何が失われるかをチケット本文に書く:

> **⚠️ Edge risk — decide before fixing**
>
> - **Edge at stake:** 全操作をキーボードで完結させる。マウス動線は意図的に用意しない
> - **Why the obvious fix would blunt it:** このペルソナが期待するドラッグ＆ドロップを足すとキーボード動線が「任意」になり、任意の動線は保守されなくなる

bug には決してこの印を付けない。欠陥はポジショニングでは正当化できないからで、
これはプロンプトのお願いではなくコードで担保している。尖りが未宣言なら、この仕組み
自体が動かない。

### severity

起票される issue には severity が付く。判断基準は「修正がどれだけ大変そうか」では
なく「エージェントが実際に観測したユーザー影響」— エージェントはコードを見られない:

| | |
| --- | --- |
| `critical` | 中心的なタスクが完全に止まる、データが失われる、出してはいけないものが出ている |
| `major` | 回避策を見つければ完了できるが、実ユーザーなら見つける前に離脱する |
| `minor` | タスクは止まらないが、明確な引っかかり・誤りがある |
| `trivial` | 見た目の問題、または今は誰も困っていない改善提案 |

issue 本文に書き込むと同時に `severity:<level>` ラベルを付けるので、トラッカー側で
絞り込める。severity はカテゴリと独立で、`ux` が `critical` にも `bug` が `trivial`
にもなりうる。4段階のどれでもなく、よくある同義語（`blocker`・`high`・`P2` など）
からも解決できない値が返ってきた場合、起票自体は行うが severity は**付けない**。
推測値を入れると、severity で並べ替えたときに捏造値と本物を見分けられなくなるため。
...
- **トリアージ結果の確認** — ラン詳細の「トリアージ」タブで、実際に起票された issue を確認できる。どの finding が1件にまとめられたか・トラッカーのリンク・起票しなかった理由・`edge-risk` が付いたチケットとそのとき懸かっていた尖りが分かる
- **指摘の採用率** — 起票した issue をチームが実際に直した（採用）か not planned で閉じた（不採用）かを、観点別・カテゴリ別に表示する。もともとペルソナ採用にフィードバックしている値を、そのまま見えるようにしたもの
- **LLM コストの横断集計** — 累計・1 run 平均・直近 30 日・トークン累計と、run ごとの推移
...
- **プロダクトの尖りの宣言** — 意図的に尖らせているところを宣言すると、それを鈍らせるチケットに triage が印を付ける（[プロダクトの尖り](#プロダクトの尖り--直すと平均に寄ってしまう指摘)を参照）
...
- コンテナや開発サーバーなどで外部公開する場合は `SHOAL_HOST=0.0.0.0` を設定する。ただしリスナーは平文 HTTP なので、`SHOAL_ALLOW_INSECURE=1` も併せて設定しない限り shoal は**起動を拒否する**（外部公開は「設定ミス」ではなく「判断」であるべきなので）。このときトークンが**必須**になる。`SHOAL_TOKEN` を設定するか、未設定なら起動時に自動生成され `?token=…` 付き URL が表示される。
