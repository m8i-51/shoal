...
function readLocalDocs(projectPath: string): string {
  ...
  for (const candidate of DOC_CANDIDATES) {
    ...
    try {
      ...
      log.info(`  [product-discovery] local doc: ${candidate} (${chunk.length} chars)`);
    } catch {
...
async function fetchGitHubReadme(githubRepo: string): Promise<string> {
  for (const branch of ["main", "master"]) {
    ...
    try {
      ...
      log.info(`  [product-discovery] GitHub README fetched (${content.length} chars, branch: ${branch})`);
...
async function gatherDocumentation(projectPath?: string): Promise<string> {
  if (projectPath) {
    ...
    if (docs) return docs;
    log.info("  [product-discovery] local docs: nothing found, falling back to GitHub");
  }
...
function saveSpec(baseUrl: string, spec: ProductSpec): void {
  ...
  log.info(`  [product-discovery] spec saved: ${filePath}`);
  ...
  if (spec.uiFeatures) {
    ...
    log.info(`  [product-discovery] UI_FEATURES saved: ${mdPath}`);
  }
}

function printSpec(spec: ProductSpec): void {
  log.info(`\n${"─".repeat(60)}`);
  log.info(`  app: ${spec.appName}`);
  log.info(`  description: ${spec.appDescription}`);
  log.info(`  users: ${spec.targetUsers}`);
  log.info(`  features:\n${spec.features.split("\n").map((l) => `    ${l}`).join("\n")}`);
  log.info(`  confidence: ${spec.confidence} / sources: ${spec.sources.join(", ")}`);
  if (spec.productEdge) {
    const edge = spec.productEdge;
    log.info(`  product edge (${edge.source}):`);
    for (const item of edge.sharpEdges) log.info(`    sharp: ${item}`);
    for (const item of edge.tradeoffs) log.info(`    trade-off: ${item}`);
  }
  if (spec.loginPath) log.info(`  login: ${spec.loginPath}`);
  const thresholds = spec.thresholdCandidates ?? [];
  if (thresholds.length > 0) {
    log.info(`  thresholds: ${thresholds.length} candidate(s)`);
    for (const t of thresholds.slice(0, 5)) {
      log.info(`    - [${t.kind}/p${t.priority}] ${t.area}: ${t.signal}`);
    }
  }
  log.info(`${"─".repeat(60)}\n`);
}
...
export async function discoverProduct(
  ...
  if (declaredEdge) log.info("  [product-discovery] keeping the team-declared product edge");
...
- When evidence is UI observation only: set confidence to low; write at most 2–3 high-level outcome drafts (no control names); treat them as Hall-editable drafts, not verified product goals`
    ...
    execute: async (input: Record<string, unknown>): Promise<string> => {
      if (t.name === "navigate_and_read") {
        ...
        try {
          ...
          log.info(`  [product-discovery] observed: ${pathArg}`);
          ...
          if (detected.formPath && !observedFormPath) {
            ...
            log.info(`  [product-discovery] login form at: ${observedFormPath}`);
          }
      ...
      if (t.name === "output_spec") {
        ...
        log.info(`  [product-discovery] spec confirmed (confidence: ${spec.confidence})`);
  ...
  log.info(`[product-discovery] done: "${spec.appName}" (confidence: ${spec.confidence})`);
  printSpec(spec);
  return spec;
}
